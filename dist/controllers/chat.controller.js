...
const getChats = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        for (var i = 0; i < chats.rows.length; i++) {
            const last_message = yield database_1.pool.query('SELECT * FROM messages WHERE  ((id_sender = $1 AND id_receiver = $2) OR (id_sender = $2 AND id_receiver = $1)) and id_request = $3 ORDER BY fecha DESC LIMIT 1', [user_id, chats.rows[i].id_customer, chats.rows[i].id_request]);
            ...
            response.push(Object.assign(Object.assign({}, chats.rows[i]), { last_message: last_message.rows.length != 0 ? last_message.rows[0] : '', customer: customer.rows[0] }));
        }
