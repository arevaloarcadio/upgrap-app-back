...
export const updateProducts = async (req: Request, res: Response): Promise<Response> => {
    ...
    try {
        if(req.file !== undefined){
            ...
            const response: QueryResult = await pool.query('UPDATE products SET address=$1, category=$2, change=$3, city=$4, description=$5, estado=$6, id_user=$7, name=$8, pais=$9, photo=$10 WHERE id = $11;', [address,category_id,to_change,city,descripcion,estado,user_id,nombre,country,filename,id]);
        }else{
            const response: QueryResult = await pool.query('UPDATE products SET address=$1, category=$2, change=$3, city=$4, description=$5, estado=$6, id_user=$7, name=$8, pais=$9 WHERE id = $10;', [address,category_id,to_change,city,descripcion,estado,user_id,nombre,country,id]);
        }
